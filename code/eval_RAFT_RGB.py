...
from RGBnets.RAFTNet import CustomRAFTNet, RAFT
...
args = RAFT.RAFT.add_model_specific_args(parser).parse_args()

if args.no_log:
  args.no_TB = True

tag = 'RGB_RAFT_' + args.tag
if not args.no_TB:
  if args.log == '':
  ...
  writer = None
  if args.log == '':
    log_path = 'runs/data_v2/RAFT_' + tag + '/'
  else:
    log_path = args.log

if args.log == '':
  fig_path = 'figures/data_v2/RAFT_' + tag + '/'
else:
...
model = CustomRAFTNet(args=args, in_channels=in_channels, time_steps=time_steps, taps=args.taps, convert='RGB').to(device)
# print(model)
