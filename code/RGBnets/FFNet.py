...
class CustomFFNet(FFN.FastFlowNet):
  def __init__(self,
               args,
               in_channels=1,
               time_steps=4,
               taps=1,
               convert='RGB'):
    super(CustomFFNet, self).__init__(args=args)
    # change input dimensionality
    if convert != 'RGB':
      self.conv1a  = FFN.convrelu(in_channels, 16, 3, 2)
    self.forward_pair = super(CustomFFNet, self).forward
