...
from MOMnet.MOM import MOMNet
from MOMnet.FlowNetS import FlowNetS
...
parser.add_argument('--LRS_patience', '--lrp', default=1000, type=int, help='patience for ReduceOnPlateau / step_size in StepLR')
...
parser.add_argument('--flownets', action='store_true', help='replace MOM_FlowNet with FlowNetS')
parser.add_argument('--div_flow', default=1.0, type=float, help='flow scaling at network output')
...
tag = 'MOM_' + args.tag + '_' + datetime.now().strftime('%b%d_%H-%M-%S')
if not args.no_TB:
  if args.log == '':
  ...
  writer = None
  if args.log == '':
    log_path = 'runs/data_v2/' + id_string + 'MOM_' + tag + '/'
  else:
    log_path = args.log

if args.log == '':
  fig_path = 'figures/data_v2/' + id_string + 'MOM_' + tag + '/'
else:
...
def train(dataloader, model, loss_fn, optimizer, plot):
  ...
  for batch, data in enumerate(dataloader):
    ...
    warped_corrs, masks = warp_correlations_n(corrs, flow)
    ...
    total_loss = 0
    if 'photo' in args.main_loss:
      total_loss += loss_photo
    if 'tof' in args.main_loss:
    ...
    if args.smooth_loss:
      flows_down = F.avg_pool2d(flow, kernel_size=4, stride=4)
      corrs_down = F.avg_pool2d(corrs, kernel_size=4, stride=4)
      loss_smooth = loss_f.edge_aware_smoothness_n(corrs_down, flows_down, lambda_param=args.smooth_loss_lambda)
...
def eval(dataloader, model, loss_fn, plot, track_valid_ratio=False):
  with torch.no_grad():
    model.eval()
    losses = {'photo': [], 'tof': [], 'tof_ref': [], 'photo_ref': [], 'tof_masked': [], 'tof_masked_ref': []}
...
def eval_save_figs(dataloader, model, loss_fn, curr_writer=None):
  with torch.no_grad():
    model.eval()
    losses = {'photo': [], 'tof': [], 'tof_ref': [], 'valid_ratio': [], 'photo_ref': [], 'tof_masked': [], 'tof_masked_ref': []}
    i = 0
    for batch, data in enumerate(dataloader):
      print('\r' + str(i), end='')
      i += 1
...
if not args.flownets:
  model = MOMNet(input_dims=in_channels, norm=None, div_flow=args.div_flow).to(device)
else:
  model = FlowNetS(input_dims=in_channels, norm=None, div_flow=args.div_flow).to(device)
# print(model)
...
if args.fix_LR_decay:
  lr_scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=args.LRS_patience, gamma=args.LRS_factor, last_epoch=-1, verbose=True)
else:
